...
- Integra seamlessly con las funcionalidades de macOS (Keychain, notificaciones, menús)
...
1. Descarga `X_0.5.0_aarch64.dmg` desde [Releases](https://github.com/686f6c61/Xcom-mac-silicon/releases)
...
# Producción (optimizado con bundles)
# Genera binarios optimizados y firmados para distribución
npm run tauri build
```

#### Artefactos generados

Después de ejecutar `npm run tauri build`, los binarios se generan en:
- **Aplicación nativa**: `src-tauri/target/release/bundle/macos/X.app`
- **Instalador DMG**: `src-tauri/target/release/bundle/dmg/X_0.5.0_aarch64.dmg`
...
Cuando cambias entre cuentas, la aplicación recarga el WebView con las credenciales correspondientes, permitiendo acceso instantáneo a diferentes perfiles. Esto es especialmente útil para usuarios que gestionan cuentas personales y profesionales, o para community managers que administran múltiples perfiles.
...
```rust
// Derivación de clave única por usuario
username => Argon2id(username, salt) => 256-bit key
...
5. Frontend ejecuta `location.reload()` para cargar X.com con las nuevas credenciales
6. WebView establece cookies y estado de sesión para la cuenta seleccionada
...
# Genera binarios optimizados en modo release
npm run tauri build
